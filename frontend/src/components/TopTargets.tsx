...
export function TopTargets({ alerts }: TopTargetsProps) {
  const { topTargets, maxCount } = useMemo(() => {
    ...
    return { topTargets: sorted, maxCount: max };
  }, [alerts]);

  return (
    <div className="panel" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div className="panel-header" style={{ borderBottom: 'none', paddingBottom: 0 }}>
        <span className="panel-title">TOP TARGETS</span>
      </div>
      <div className="panel-content" style={{ flex: 1, padding: 'var(--space-4)', display: 'flex', flexDirection: 'column', gap: '12px', marginTop: 'var(--space-2)' }}>
        {topTargets.length === 0 ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <span style={{ color: 'var(--text-secondary)', fontSize: '12px', marginTop: '6px' }}>Target data unavailable.</span>
          </div>
        ) : (
