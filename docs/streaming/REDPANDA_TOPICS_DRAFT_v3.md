# SIH26145 — REDPANDA TOPIC DESIGN (DRAFT v3)
...
Partitioning is **PROPOSED and intentionally not locked in this document**.

### Candidate A — `src_ip` for all raw event topics
...
### Candidate B — hybrid source / relationship locality
...
TLS/C2 beaconing is based on repeated connections between a source and destination over time. A compound `(src_ip, dst_ip)` key can keep a beacon relationship localized even though each connection has a different `connection_id`.
...
Do not finalize Candidate A, Candidate B, or another key until the **Feature/Window Schema** is drafted and the aggregation entities/windows are known.

The Feature/Window design should evaluate:

```text
source-centric aggregation
destination-centric aggregation
source→destination relationship aggregation
connection-level correlation
partition skew
cross-partition state requirements
```
...
- [ ] Final partition-key strategy
  - `src_ip` for all topics
  - or hybrid `src_ip` / `(src_ip, dst_ip)` for TLS
  - or another strategy justified by Feature/Window aggregation
...
- [ ] Partition key is evaluated against Feature/Window aggregation.
...
| `src_ip` partitioning | **PROPOSED** |
| TLS `(src_ip, dst_ip)` alternative | **PROPOSED** |
...
**DRAFT v3 — awaiting project-lead/team review.**

No Redpanda topics should be implemented in code from this document until the proposed design is explicitly reviewed and approved.
