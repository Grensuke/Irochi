...
- Locked four foundational items via explicit project-lead approval (Task 16a):
...
The atomic counter strategy (`INCR`/`HINCRBY` against time-bucketed counters, no read-modify-write) is now **LOCKED** as the baseline approach. Detailed state model, key shapes, and evaluation flow are defined in §9a.2. Logical destination-key sharding remains OPEN as a future benchmark-driven optimization (§9a.13).

### 16.2 Pair-state cardinality control

The Tier 1 / Tier 2 split (gating vs feature-producing) is now **LOCKED** in terms of existence and roles. Detailed state model, key shapes, promotion flow, and eviction considerations are defined in §9a.5. Promotion thresholds, eviction policy, and memory budget remain OPEN.
...
| Snapshot-not-delta rule (§6) | **LOCKED PROCESSING INVARIANT** — upgraded in v7; required for state-to-feature boundary (§9a.9); consistent across all revisions since introduction |
...
| DDoS atomic counter strategy (INCR/HINCRBY baseline) | **LOCKED** — upgraded in v7; required for DDoS destination state model (§9a.2/§9a.13); sharding remains OPEN as optimization |
| Directional pair key `(src_ip, dst_ip)`, not canonicalized (§3) | **LOCKED** — added in v7; required for Tier 1/Tier 2 key shapes (§9a.5); unchanged since introduction |
| Pair-state Tier 1/Tier 2 split: existence and roles | **LOCKED** — split from prior "Pair-state two-tier tracking" row in v7; Tier 1 = gating only, Tier 2 = feature-producing; required for §9a.5 |
...
Items marked LOCKED in this table have explicit project-lead approval. PROPOSED items are structural baselines subject to review. OPEN items require benchmark/dataset resolution or depend on upstream decisions not yet made.
...
Four foundational items were explicitly locked with project-lead approval:
